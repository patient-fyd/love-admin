...
const { loading, items, total, error, fetch } =
  usePaginatedEndpoint<CostReportItem>('/admin/ai/cost/report');
...
<template>
  <div class="p-4">
    <NCard title="成本报表" :loading="loading">
      <NResult
        v-if="error"
        status="error"
        :title="error"
        description="检查 /admin/ai/cost/report 接口"
      />
